...
def model_plan() -> dict[str, Any]:
    return {
        "status": "MODEL_BACKEND_NOT_IMPLEMENTED_OR_AUTHORIZED",
        "preflight": {
            ...
            "command_after_separate_go": (
                "python3 $TMP/openramble-dual-cache-falsifier/run.py "
                "model-preflight --worker <release-same-artifact-worker> "
                "--fixture <frozen-long-f32le> --offsets 0,25,50,75,100 "
                "--n-per-arm 2 --trace-coreml <result.trace>"
            ),
            "required_measurements": {
                ...
                "active_kill": (
                    "prediction-start marker -> SIGKILL -> waitpid -> last S ANE event end; "
                    "F request starts only after reap"
                ),
                "cold_reload": (
        ...
        "hard_gates": {
            ...
            "coreml_trace_requires_no_speculative_tail_after_waitpid": True,
            "active_kill_reap_deadline_ms": 250,
            "reject_if_native_ane_survives_reap": True,
            "require_same_ane_route_for_speculative_and_foreground": True,
...
def parse_arguments() -> argparse.Namespace:
    ...
    # These names deliberately fail closed until the Swift model worker exists
    # and root grants a separate model-lane GO.
...
async def async_main(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    raise HarnessFailure(
        "model execution is fail-closed: implement/review the same-artifact Swift worker "
        "and obtain a separate explicit CoreML GO"
    )
