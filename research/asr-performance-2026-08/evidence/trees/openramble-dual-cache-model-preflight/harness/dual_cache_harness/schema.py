...
class ModelExecutionIdentity:
    ...
    coreml_binary_identity_kind: str
    ...
    def __post_init__(self) -> None:
        fields = (
            ...
            self.coreml_binary_identity_kind,
        )
...
class ClosedWindowDescriptor:
    ...
    stable_through_sample_count: int
    ...
    def __post_init__(self) -> None:
        ...
        if self.stable_through_sample_count < 0:
            raise ValueError("negative closed-window stability watermark")
        ...
        if self.earliest_safe_prefix_sample_count < self.stable_through_sample_count:
            raise ValueError("closed-window exposure precedes its stability watermark")
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "ClosedWindowDescriptor":
        ...
        for name in ("emit_tokens_after_frame", "initial_time_index_override"):
            raw = value.get(name)
        ...
        return cls(
            **{
                ...
                if field.name not in optional_ints
            },
            **optional_ints,
        )
...
class PrimaryTokenAudit:
    ...
    def __post_init__(self) -> None:
        if self.token_id < 0:
            raise ValueError("negative primary token id")
        if not 0 <= self.start_time_bits <= 0xFFFF_FFFF_FFFF_FFFF:
            raise ValueError("start_time_bits is not Float64 bits")
        if not 0 <= self.end_time_bits <= 0xFFFF_FFFF_FFFF_FFFF:
            raise ValueError("end_time_bits is not Float64 bits")
        if not 0 <= self.confidence_bits <= 0xFFFF_FFFF:
            raise ValueError("confidence_bits is not Float32 bits")
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "PrimaryTokenAudit":
        return cls(
            token=str(value["token"]),
            token_id=int(value["token_id"]),
            start_time_bits=int(value["start_time_bits"]),
            end_time_bits=int(value["end_time_bits"]),
            confidence_bits=int(value["confidence_bits"]),
        )
...
class SerializedClosedWindowCache:
    ...
    def metadata_dict(self) -> dict[str, Any]:
        return {
            ...
            "language_raw_value": self.language_raw_value,
    ...
    def from_parts(
        ...
        return cls(
            ...
            language_raw_value=metadata.get("language_raw_value"),
    ...
    def validate(
        ...
        *,
        ...
        for record in records:
            ...
            if record.language_raw_value != expected_identity.language_hint:
                raise CacheImportError("cache_language_mismatch")
...
class AuditedResult:
    transcript: str
    primary_tokens: tuple[PrimaryTokenAudit, ...]
    word_records_json: bytes
    vocabulary_outcome: str
    candidate_regions_json: bytes
    audio_duration_bits: int

    def __post_init__(self) -> None:
        if not 0 <= self.audio_duration_bits <= 0xFFFF_FFFF_FFFF_FFFF:
            raise ValueError("audio_duration_bits is not Float64 bits")
        for label, payload in (
    ...
    def to_dict(self, *, include_sensitive_text: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            ...
            "audio_duration_bits": self.audio_duration_bits,
        }
        if include_sensitive_text:
            ...
            payload["token_strings_sha256"] = hashlib.sha256(
                canonical_json_bytes([item.token for item in self.primary_tokens])
            ).hexdigest()
        return payload
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "AuditedResult":
        if "transcript" not in value:
            raise ValueError("an in-memory parity result must include exact text")
        return cls(
            transcript=str(value["transcript"]),
            primary_tokens=tuple(
                PrimaryTokenAudit.from_dict(item) for item in value["primary_tokens"]
            ),
            ...
            audio_duration_bits=int(value["audio_duration_bits"]),
        )
